...
namespace Jellyfin.Plugin.SmartLists.Services.Playlists
{
    ...
    public class PlaylistStore : ISmartListStore<SmartPlaylistDto>
    {
        ...
        {
            var filePaths = _fileSystem.GetAllSmartListFilePaths();
            ...
            foreach (var filePath in filePaths)
            {
                try
                {
                    var dto = await LoadPlaylistAsync(filePath).ConfigureAwait(false);

                    // Only return playlists (filter out collections)
                    if (dto != null && dto.Type == Core.Enums.SmartListType.Playlist)
                    {
                        validPlaylists.Add(dto);
                    }
                    // Handle legacy files without Type field - assume they're playlists
                    else if (dto != null && dto.Type == 0) // Default enum value
                    {
                        // Legacy file - set type to Playlist for backward compatibility
        ...
        private async Task<SmartPlaylistDto?> LoadPlaylistAsync(string filePath)
        {
            ...
            // Handle backward compatibility: if Type is not set, default to Playlist
            if (dto != null && dto.Type == 0)
            {
                dto.Type = Core.Enums.SmartListType.Playlist;
            }
